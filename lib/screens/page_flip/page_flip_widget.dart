...
import 'package:mi_insights/screens/page_flip/builders/builder.dart';

class PageFlipController {
  PageFlipWidgetState? _state;

  void attach(PageFlipWidgetState state) {
    _state = state;
  }

  Future<void> goToPage(int index) async {
    await _state?.goToPage(index);
  }

  Future<void> nextPage() async {
    await _state?.nextPage();
  }

  Future<void> previousPage() async {
    await _state?.previousPage();
  }
}
...
class PageFlipWidgetState extends State<PageFlipWidget>
  ...
  void initState() {
    super.initState();
    widget.controller.attach(this); // Attach the controller
    _setUp();
  }
  ...
  Future nextPage() async {
    ...
    if (mounted) {
      setState(() {
        pageNumber++;
        widget.onPageChanged?.call(pageNumber); // Invoke callback
      });
    }

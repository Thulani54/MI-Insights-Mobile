...
class _CollectionsReportState extends State<CollectionsReport> {
                      ...
                      child: Container(
                          ...
                          child: isLoadingStackedBarGraph == true
                              ? _selectedButton == 1
                                  ...
                                  : _selectedButton == 2
                                      ? buildBarChart2(jsonString)
                                      : buildBarChart3(jsonString)
                              : Center(
                                  child: Center(
                                  child: Padding(
                                    padding: const EdgeInsets.all(8.0),
                                          ...
                                          child: Row(
                                            children: [
                                              Expanded(
                                                child: Center(
                                                  child: Text(
                                                    "R${salesbyagent[index].sales.toStringAsFixed(2)}",
                                                    textAlign: TextAlign.center,
                                                  ),
                                                ),
                                              ),
                          ...
                          child: Padding(
                            ...
                            child: charts.BarChart(
                              ...
                              primaryMeasureAxis: new charts.NumericAxisSpec(
                                renderSpec: new charts.NoneRenderSpec(),
                                viewport: new charts.NumericExtents(0, maxY5),
                              ),
                              domainAxis: new charts.OrdinalAxisSpec(
  ...
  void updateSectionsList(
    ...
    for (var i = 0; i < allSales.length; i++) {
      var status = allSales[i].quoteStatus;
      var requestStatus = "label-primary";

      var title = (status == 'Inforced' || status == 'Incepted')
          ? (status + ' (Policy Number: ' + allSales[i].policyNumber + ')')
          : (status == 'Failed')
              ? (status + ' (' + allSales[i].mipDescription + ')')
              : (status + ' (Reference Number: ' + allSales[i].reference + ')');

      /*   if (salesView == status ||
          salesView == 'Total Sales' ||
          (salesView == 'New' && status == 'Updated') ||
          (salesView == 'New' && status == 'Incepted')) {
        // Your logic
      }*/
      ...
      await http.get(
        ...
        if (response.statusCode != 200) {
          ...
          for (var element in salesbybranch) {
            if (_section_id < 5) {
              //print("_section_id ${_section_id}");
              pieData.add(PieChartSectionData(
                color: samplecolors[_section_id],
                value: element.sales.toDouble(),
                title: element.sales.toString(),
                radius: 60,
          ...
          bardata5.add(charts.Series<OrdinalSales, String>(
              ...
              measureFn: (OrdinalSales sale, _) =>
                  double.parse(sale.sales.toStringAsFixed(2)),
              data: ordinary_sales,
              // Set a label accessor to control the text of the bar label.
              labelAccessorFn: (OrdinalSales sale, _) =>
                  'R${double.parse(sale.sales.toStringAsFixed(2))}'));
  ...
  Widget buildBarChart2(String jsonString) {
    List<BarChartGroupData> barChartData = [];
    if (_selectedButton == 2) {
      // barChartData = processDataForCollectionsCount(jsonString);
    } else if (_selectedButton == 1) {
      barChartData = processDataForCollectionsCountDaily(jsonString);
    }
    Widget barsWidget = Padding(
      padding: const EdgeInsets.all(8.0),
      child: Card(
        ...
        child: AspectRatio(
          ...
          child: Padding(
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                final double barsSpace = 4.0 * constraints.maxWidth / 200;
                final double barsWidth = 8.0 * constraints.maxWidth / 40;
                      ...
                      titlesData: FlTitlesData(
                        ...
                        leftTitles: AxisTitles(
                          sideTitles: SideTitles(
                            ...
                            getTitlesWidget: (value, meta) {
                              return Text(
                                formatLargeNumber2(value.toString()),
  ...
  Widget buildBarChart3(String jsonString) {
    List<BarChartGroupData> barChartData = [];
    if (_selectedButton == 2) {
      //barChartData = processDataForCollectionsCount(jsonString);
    } else if (_selectedButton == 1) {
